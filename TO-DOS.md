...
## Slack Alert Channel - 2026-02-12 23:30

- **Add Slack alert channel** - Block Kit formatted messages via Slack Web API (`chat.postMessage`). Severity-colored attachments, masked sample previews, groupable by table/rule. **Problem:** No native Slack integration — users must configure raw webhooks without rich formatting. **Files:** `src/alerts/mod.rs:18-49`, `src/alerts/config.rs:1-73`, `src/alerts/dispatcher.rs`. **Solution:** Add `SlackChannel` variant to `AlertChannel` enum, `SlackConfig` to `AlertsConfig` (token, channel, optional username/icon). Reuse `reqwest` (already a dep).
